...
export default class AccModal extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      cardErr: "#fff",
      accountbank: this.props.accountbank
    };
  ...
  check() {
    ...
    if (
      this.props.accountbank.length < 2 ||
      ...
      this.props.accountbank == "058" ||
      this.props.accountbank == "011"
    ) {
  ...
  confirmOtp() {
    ...
    this.props.rave
      .validateAccount({
        transactionreference: this.state.flwRef,
        otp: this.state.otp
      })
      .then(res => {
        if (res.data.status.toUpperCase() === "SUCCESSFUL") {
          ...
          this.props.rave
            .verifyTransaction(res.data.txRef)
            .then(resp => {
              this.props.onSuccess(resp);
              if (
                resp.data.status.toUpperCase() === "SUCCESSFUL" &&
                resp.data.chargecode === "00"
              ) {
    ...
    this.props.rave
      ...
      .then(res => {
        ...
        if (res.data.status.toUpperCase() === "SUCCESSFUL") {
          ...
          res.data.authurl.toUpperCase() === "NO-URL"
        ) {
          this.setState({
            flwRef: res.data.flwRef,
            otpModal: true,
            loading: true,
            chargeResponseMessage: res.data.validateInstruction
  ...
  render() {
    ...
    let btnText = 
        `FUND ${this.props.currency} ${this.props.amount}`
          ...
          <View style={styles.input}>
            ...
            <View style={{ paddingVertical: 10, flexDirection: "row" }}>
              <DatePicker
                ...
                onDateChange={date =>
                  this.setState({ dob: date, selectedDate: true })
                }
              />
            </View>
          </View>
    ...
    if (this.props.accountbank == "044") {
      logo = (
        <View style={styles.logo}>
          <Image source={require("../../assets/icons/access.png")} />
        </View>
      );
    } else if (this.props.accountbank == "101") {
      logo = (
        <View style={styles.logo}>
          <Image source={require("../../assets/icons/pb.png")} />
        </View>
      );
      ...
      logo = (
        <View style={styles.logo}>
          <Image
            source={require("../../assets/icons/sterlingBankLogoWk.png")}
          />
        </View>
      );
    } else if (this.props.accountbank == "057") {
      logo = (
        <View style={styles.logo}>
          <Image source={require("../../assets/icons/zenithLogo.png")} />
        </View>
      );
    }
    return (
      ...
      >
        ...
        <VBVSecure
          vbvModal={this.state.vbvModal}
          url={this.state.vbvurl}
          confirm={this.confirmVBV}
        />
        <View style={{ flex: 1 }}>
          {logo}
          <View style={styles.formGroup}>
